...
def update_focus(state, observer, event_radar, tactical, scan_rows, *, now=None):
    ...
    for symbol, item in list(focus.items()):
        ...
        if trow:
            ...
            item["entry_window_state"] = trow.get("entry_window_state")
            item["route_degraded_age_s"] = trow.get("route_degraded_age_s")
            item["five_minute_witness_supportive"] = trow.get("five_minute_witness_supportive")
