...
RESEARCH_BUILD_ID = "2026-09-01-INSTITUTIONAL-V9.3.2-RESEARCH-UI-ISOLATION"
...
def build_feature_frame(df, timeframe="15minute", oi_series=None, index_df=None, sector_df=None, sector_rank_series=None, futures_df=None, daily_oi_series=None):
    ...
    """
    import pandas as pd
    from . import early_signal, indicators, stock_in_play, v93_component_lab
    ...
    # ATR is a first-class feature in V9.3 because "price flat" must be
    # volatility-normalised rather than an arbitrary percentage threshold.
    out["atr"] = pd.to_numeric(series["atr"], errors="coerce")
    close_for_atr = pd.to_numeric(df["close"], errors="coerce")
    out["atr_pct"] = out["atr"] / close_for_atr.replace(0, np.nan) * 100.0
    ...
    if timeframe == "15minute":
        lag = close_num.shift(4)
        idx = pd.DatetimeIndex(df.index)
        same_session = pd.Series(idx.normalize(), index=df.index).eq(
            pd.Series(idx.normalize(), index=df.index).shift(4)
        )
        out["price_move_60m_atr"] = ((close_num - lag).abs() / out["atr"].replace(0, np.nan)).where(same_session)
    else:
        out["price_move_60m_atr"] = (close_num.diff().abs() / out["atr"].replace(0, np.nan))

    # Point-in-time DAILY continuous-futures OI is the long-history swing
    # evidence stream.  It is stamped at the completed session close before
    # being mapped to intraday bars, so a morning signal never sees that day's
    # yet-unfinished OI.  This is separate from current-contract intraday OI.
    daily_pti = v93_component_lab.point_in_time_daily_oi_features(
        daily_oi_series, df.index, min_obs=early_signal.MIN_BASELINE_OBS,
        window=early_signal.BASELINE_DAYS,
    )
...
def _replay_breakout_feature_frame(df, features, symbol, cost_pct=0.05, slippage_pct=0.02,
    ...
    """
    from . import stock_in_play, v93_component_lab
    ...
    def _event_from_row(row, pos, direction, classified):
        ...
        event = {
            ...
            "price_move_60m_atr": row.get("price_move_60m_atr"),
            "oi_z": row.get("oi_z"),
    ...
    def _v93_movement_event(row, pos, event_type):
        if setup_timeframe != "15minute" or execution_df is not None:
            return None
        atr = _py(features["atr"].iloc[pos]) if "atr" in features.columns else None
        ...
        return {
            "symbol": symbol,
            "signal_time": df.index[pos].isoformat(),
            "entry_time": df.index[pos + 1].isoformat(),
            "direction": None,
            "v93_event_type": event_type,
            "movement_outcomes": movement,
            "oi_z": row.get("oi_z"),
            "oi_chg_30m_pct": row.get("oi_chg_30m_pct"),
            "oi_chg_60m_pct": row.get("oi_chg_60m_pct"),
            "oi_acceleration": row.get("oi_acceleration"),
    ...
    short_seed_was_active = False
    silent_state_was_active = False
    compression_state_was_active = False
    last_silent_seed_pos = None
    ...
    for pos in range(len(df)):
        ...
        if fast_v8 and setup_timeframe == "15minute" and pos + 1 < len(df):
            session_key = pd.Timestamp(df.index[pos]).date().isoformat()
            # Fixed daily baseline: the first completed bar at/after 11:00.
            # One sample per symbol/session avoids manufacturing huge N from
            # overlapping 15-minute observations.
            ts = pd.Timestamp(df.index[pos])
            if session_key not in baseline_session_seen and (ts.hour > 11 or (ts.hour == 11 and ts.minute >= 0)):
                baseline = _v93_movement_event(row, pos, "baseline")
                if baseline is not None:
                    v9_playbook_events.append(baseline)
                    baseline_session_seen.add(session_key)

            silent_state = v93_component_lab.is_silent_oi_state(row)
            silent_rise = bool(silent_state and not silent_state_was_active)
            silent_state_was_active = silent_state
            if silent_rise and session_key not in silent_session_seen:
                seed = _v93_movement_event(row, pos, "silent_oi")
                if seed is not None:
                    v9_playbook_events.append(seed)
                    silent_session_seen.add(session_key)
                    last_silent_seed_pos = pos
                    last_silent_seed_compression = row.get("compression_score")

            comp_val = row.get("compression_score")
            compression_state = bool(comp_val is not None and np.isfinite(comp_val) and float(comp_val) >= 60.0)
            compression_rise = bool(compression_state and not compression_state_was_active)
            compression_state_was_active = compression_state
            if compression_rise:
                comp_event = _v93_movement_event(row, pos, "compression_onset")
                if comp_event is not None:
                    v9_playbook_events.append(comp_event)
            ...
            if daily_anomaly and session_key not in daily_oi_session_seen:
                ...
                if daily_event is not None:
                    v9_playbook_events.append(daily_event)
                    daily_oi_session_seen.add(session_key)

            # Expire a silent setup if no fresh ignition appears within one hour.
            if last_silent_seed_pos is not None and pos - last_silent_seed_pos > 4:
                last_silent_seed_pos = None
                last_silent_seed_compression = None
        ...
        if long_seed_rise:
            ...
            if accumulation_event is not None:
                ...
                accumulation_event["v93_event_type"] = "long_buildup"
        ...
        short_seed_rise = bool(short_seed and not short_seed_was_active)
        short_seed_was_active = short_seed
        if short_seed_rise:
            short_event = _event_from_row(row, pos, "Bearish", {})
            if short_event is not None:
                short_event["v93_event_type"] = "short_buildup"
                short_event["fresh_breakout"] = False
                short_event["breakout_source"] = None
                v9_playbook_events.append(short_event)
        ...
        if direction in ("Bullish", "Bearish") and row.get("fresh_breakout") is True and pos + 1 < len(df):
            ...
            if event is not None:
                event["v93_event_type"] = "fresh_breakout"
                lead_bars = None
                if last_silent_seed_pos is not None and 1 <= pos - last_silent_seed_pos <= 4:
                    lead_bars = int(pos - last_silent_seed_pos)
                event["v93_silent_oi_lead"] = lead_bars is not None
                event["v93_silent_oi_lead_bars"] = lead_bars
                event["v93_silent_compression_score"] = last_silent_seed_compression if lead_bars is not None else None
                event["v93_absolute_regime_aligned"] = v93_component_lab.absolute_regime_aligned(
                    direction, row.get("index_ret_8_pct")
                )
                event["v93_trial13_candidate"] = v93_component_lab.is_trial13_candidate(event)
                if lead_bars is not None:
                    # Trial 13 consumes the first ignition after a silent setup;
                    # later breakouts are not independent attempts.
                    last_silent_seed_pos = None
                    last_silent_seed_compression = None
