...
SLOTS = (("OPEN_STABLE", dt.time(9, 30)), ("MIDDAY", dt.time(13, 0)), ("PRE_CAS", dt.time(15, 10)), ("POST_CAS", dt.time(15, 37)))
GRACE_MINUTES = 7
...
def _slot_health(state, now):
    ...
    names = set(captured if isinstance(captured, (list, tuple, set)) else captured.keys())
    out = {}
    for name, clock in SLOTS:
        ...
        status = "CAPTURED" if name in names else ("MISSED" if now > deadline else "PENDING")
...
def operational_health(*, v12_snapshot_file, v12_state_file, v121_root, v121_state_file, backup_state_file, now, storage_mode, connection_state=None):
    ...
    v12 = _read(v12_state_file); v121 = _read(v121_state_file); backup = _read(backup_state_file)
    day = str(v121.get("current_day") or now.date().isoformat()); root = Path(v121_root)
    micro = root / f"{day}_micro.jsonl"; depth = root / f"{day}_depth.jsonl"
