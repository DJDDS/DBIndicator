...
def _captured_today(captured,name,day):
    if isinstance(captured,dict):
        if name not in captured:return False
        value=captured.get(name)
        # If a timestamp is available, never let yesterday's state certify today.
        if value:
            try:return dt.datetime.fromisoformat(str(value).replace('Z','+00:00')).date()==day
            except ValueError:return False
        return False
    # Legacy list state has no date evidence, so fail closed rather than certify it.
    return False
def _slot_health(state,now):
    ...
    for name,clock in SLOTS:
        ...
        status='CAPTURED' if _captured_today(captured,name,now.date()) else ('MISSED' if now>deadline else 'PENDING')
...
def operational_health(*,v12_snapshot_file,v12_state_file,v121_root,v121_state_file,backup_state_file,now,storage_mode,connection_state=None):
    """Aggregate evidence without mutating recorder state or data."""
    v12=_read(v12_state_file); v121=_read(v121_state_file); backup=_read(backup_state_file); day=str(v121.get('current_day') or now.date().isoformat()); root=Path(v121_root); micro=root/f'{day}_micro.jsonl'; depth=root/f'{day}_depth.jsonl'
    return {'observed_at':now.isoformat(timespec='seconds'),'storage_mode':storage_mode,
      ...
      'v121':{'status':v121.get('status'),'active_expiry':v121.get('active_expiry'),'atm_strike':v121.get('atm_strike'),'token_count':v121.get('token_count'),'session_last_tick_at':v121.get('session_last_tick_at'),'last_tick_age_seconds':_age(now,v121.get('session_last_tick_at')),'last_micro_write_at':v121.get('last_micro_write_at'),'last_depth_write_at':v121.get('last_depth_write_at'),'micro_snapshot_count':int(v121.get('micro_snapshot_count') or 0),'depth_snapshot_count':int(v121.get('depth_snapshot_count') or 0),'last_write_error':v121.get('last_write_error'),'micro_file_bytes':_size(micro),'depth_file_bytes':_size(depth),'micro_gzip_bytes':_size(str(micro)+'.gz'),'depth_gzip_bytes':_size(str(depth)+'.gz')},
