...
def dashboard():
    ...
    event_early_radar = event_driven_early_radar(
...
def api_v8_dashboard():
    ...
    payload["event_early_radar"] = event_driven_early_radar(
        base_radar, state.get("v122b_tactical") or {}
    )
...
def backtest_page():
    ...
    return render_template(
        ...
        v111_state=backtest.get_v111_development_state(),
...
@app.route("/api/v111/development/start", methods=["POST"])
@require_dashboard_password
def api_v111_development_start():
    return jsonify(backtest.start_v111_development_lab())
