...
up on the last CLOSED candle for a symbol.
...
1. Telegram - a message pushed to your phone, works even if the
   ...
   not configured.
2. In-app - every fresh signal is kept in a small rolling log regardless
   of Telegram config; the dashboard polls /api/alerts/recent and shows
   a toast + beep for anything new while the tab is open.
...
def _format_message(r, timeframe):
    ...
    return (
        f"{arrow} {r['symbol']} - {r['fresh_signal']} confluence on {timeframe}\n"
...
def process_scan_results(results, timeframe):
    ...
    configured) and always records fresh signals to the in-app log."""
    for r in results or []:
