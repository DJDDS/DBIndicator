...
def test_v950_runner_is_daily_only_and_defaults_to_three_years(monkeypatch):
    ...
    assert out['research']['build'].startswith('2026-09-01-INSTITUTIONAL-V9.5.0')
    assert progress[-1][:2] == (2,2)
    assert all(call[1] == 'day' for call in kite.calls)
    oi_calls = [c for c in kite.calls if c[3]]
    assert oi_calls and all(c[2] is True for c in oi_calls)
...
def test_v950_runner_discloses_missing_point_in_time_controls():
    _clear_scanner_caches()
    out = backtest.run_v95_daily_oi_evidence(FakeKite(), symbols=['AAA'])
    r = out['research']
    assert r['controls']['mwpl_control'] == 'UNAVAILABLE'
    assert r['controls']['historical_membership'] == 'CURRENT_UNIVERSE_REPLAY_SURVIVORSHIP_BIAS'
    assert r['controls']['lot_size_normalization'] == 'UNAVAILABLE_DISCLOSED'
    assert r['controls']['atm_iv_control'] == 'UNAVAILABLE_NOT_FABRICATED'
    assert r['status'].startswith('INCONCLUSIVE_')


def test_v950_runner_accepts_integrity_series_and_filters_membership():
    _clear_scanner_caches()
    kite = FakeKite()
...
def test_v950_runner_checkpoints_each_symbol_and_resumes_without_refetch(tmp_path):
    _clear_scanner_caches()
    first = FakeKite()
...
def test_v950_primary_runner_enforces_three_year_minimum_window():
    _clear_scanner_caches()
    out = backtest.run_v95_daily_oi_evidence(FakeKite(), symbols=['AAA'], days=800)
    assert out['days'] == 1095


def test_v950_three_year_partition_keeps_v94_180day_discovery_window_in_locked_final():
    _clear_scanner_caches()
    out = backtest.run_v95_daily_oi_evidence(FakeKite(), symbols=['AAA'])
